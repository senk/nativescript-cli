...
export class UpdateCommand implements ICommand {
	...
	public async execute(args: string[]): Promise<void> {
		...
		const fsWatcher = await this.$fileSysemWatcher.startWatcher({ paths: [process.cwd()], options: { ignoreInitial: true } });
		fsWatcher.on("all", (...args: any[]) => {
			console.log("event all rased")
			console.log(args);
			console.log("$$$$$$$$$$");
		});

		setTimeout(async () => {
			console.log("lets kill it with fire");
			await this.$fileSysemWatcher.stopWatcher(fsWatcher);
			console.log("########");
		}, 10000);
